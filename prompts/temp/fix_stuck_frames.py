#!/usr/bin/env python3
"""
Script to fix frames stuck at "4 - Audio Transcribed" when parent is at "7 - Generating Embeddings"
"""
...
def fix_stuck_frames():
    """Fix frames that are stuck at '4 - Audio Transcribed' when parent is at '7 - Generating Embeddings'."""
    try:
        token = config.get_token()
        
        print("🔧 Fixing stuck frames...")
        ...
        for record in frame_records:
            ...
            # Check if frame is stuck at "4 - Audio Transcribed" but parent is at "7 - Generating Embeddings"
            if (frame_status == "4 - Audio Transcribed" and 
                parent_status == "7 - Generating Embeddings"):
                stuck_frames.append({
                    "frame_id": frame_id,
        ...
        if not stuck_frames:
            print("✅ No stuck frames found to fix")
            return
        
        print(f"🔧 Found {len(stuck_frames)} stuck frames to fix:")
        for frame in stuck_frames:
            print(f"  -> {frame['frame_id']}: {frame['frame_status']} (Parent: {frame['parent_id']} - {frame['parent_status']})")
        
        # Ask for confirmation
        response = input(f"\nProceed to fix {len(stuck_frames)} frames? (y/N): ")
        if response.lower() != 'y':
        ...
        if fixed_count > 0:
            print(f"✅ Successfully fixed {fixed_count} stuck frames!")
