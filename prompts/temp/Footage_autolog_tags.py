...
FIELD_MAPPING = {
    ...
    "tags_list": "TAGS_List",  # NEW: Added tags field
...
def load_tags():
    ...
    try:
        ...
        print(f"  -> Loaded {len(tags)} approved footage tags with descriptions")
        return tags
    except Exception as e:
        print(f"  -> WARNING: Failed to load footage tags file: {e}")
        return []
...
def generate_video_description(client, frames_data, footage_data, prompts, tags, token):
    ...
    try:
        ...
        # Parse title, description, date, audio_type, location, and tags from JSON response
        title, description, date, audio_type, location, returned_tags = parse_json_response(final_response)
        ...
        return title, description, date, audio_type, location, csv_data, returned_tags
...
def parse_json_response(response_text):
    """Parse title, description, date, audio_type, location, and tags from JSON API response."""
    title = ""
    description = ""
    date = ""
    ...
    try:
        ...
        try:
            ...
            print(f"  -> JSON parsing successful: title='{title[:50]}...', description='{description[:50]}...', date='{date}', audio_type='{audio_type}', location='{location}', tags={len(tags)} items")
            ...
            for line in lines:
                ...
                elif not title and not line.lower().startswith(('description:', 'date:', 'audio_type:', 'location:', 'tags:')):
                    title = line
                elif title and not description and not line.lower().startswith(('date:', 'audio_type:', 'location:', 'tags:')):
                    description = line
        ...
        # Location and tags can remain empty if not provided
            
        print(f"  -> Final parsed values: title='{title}', description='{description[:50]}...', date='{date}', audio_type='{audio_type}', location='{location}', tags={tags}")
    ...
    return title, description, date, audio_type, location, tags
    ...
    try:
        ...
        if not frame_records:
            ...
            field_data = {
                ...
                FIELD_MAPPING["tags_list"]: ""  # Empty tags
            }
            config.update_record(token, "FOOTAGE", record_id, field_data)
        ...
        title, description, date, audio_type, location, csv_data, returned_tags = generate_video_description(client, frames_data, footage_data, prompts, tags, token)
        ...
        tags_for_fm = ", ".join(returned_tags) if returned_tags else ""
        
        # Update the footage record with title, description, date, audio_type, location, tags, and video events
        field_data = {
            FIELD_MAPPING["description"]: description,
            FIELD_MAPPING["date"]: date,
            FIELD_MAPPING["audio_type"]: audio_type,
            FIELD_MAPPING["tags_list"]: tags_for_fm,  # NEW: Store tags
