...
def remove_child_frames(footage_id, token):
    """Remove all child frame records for a footage ID."""
    try:
        print(f"🗑️ Removing child frames for {footage_id}...")
        ...
        print(f"  📋 Found {len(frame_records)} frames to remove")
        ...
        print(f"  ❌ Error removing frames for {footage_id}: {e}")
        return False
...
def main():
    ...
    try:
        ...
        print(f"🧪 Starting comprehensive test for {len(footage_ids)} footage IDs")
        print(f"📋 Test IDs: {footage_ids}")
        print("=" * 60)
        
        # Step 1: Reset all footage statuses
        print("\n🔄 STEP 1: Resetting footage statuses...")
        reset_success = 0
        for footage_id in footage_ids:
        ...
        # Step 2: Remove all child frames
        print("\n🗑️ STEP 2: Removing child frames...")
        frame_removal_success = 0
        for footage_id in footage_ids:
            if remove_child_frames(footage_id, token):
                frame_removal_success += 1
        
        print(f"✅ Removed frames for {frame_removal_success}/{len(footage_ids)} footage IDs")
        ...
        print(f"  Frame removals: {frame_removal_success}/{len(footage_ids)}")
