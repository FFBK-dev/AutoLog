#!/usr/bin/env python3
"""
LF AutoLog Poller - Step 4: Gemini Multi-Image Analysis

Independent polling loop for Gemini API multi-image analysis.
Polls every 15 seconds (API rate-limited, medium priority).
"""
...
STEP_CONFIG = {
    ...
    "poll_interval": 15  # Medium priority, API rate-limited
}
...
def process_single_record(record, token):
    ...
    try:
        ...
        # This prevents Step 5 from picking up before Gemini completes
        ...
        if success:
            ...
            if update_status(record_id, token, STEP_CONFIG["next_status"]):
                tprint(f"  -> ✅ {footage_id}: Gemini analysis completed")
            else:
                tprint(f"  -> ⚠️ {footage_id}: Gemini done but status update failed")
        else:
...
def main():
    ...
    try:
        ...
        while True:
            ...
            if records:
                tprint(f"🔍 Found {len(records)} LF records ready for Gemini")
                
                # Process in parallel (limited by max_workers for API limits)
