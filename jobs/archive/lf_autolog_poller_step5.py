#!/usr/bin/env python3
"""
LF AutoLog Poller - Step 5: Create Frame Records

Independent polling loop for creating FileMaker frame records from Gemini results.
Polls every 20 seconds (medium speed process).
"""
...
STEP_CONFIG = {
    ...
    "timeout": 600,
    "max_workers": 3,
    "poll_interval": 20  # Medium speed process
}
    ...
    try:
        ...
        if success:
            ...
            if update_status(record_id, token, STEP_CONFIG["next_status"]):
                tprint(f"  -> ✅ {footage_id}: Frame records created")
            else:
                tprint(f"  -> ⚠️ {footage_id}: Frames created but status update failed")
        else:
...
def main():
    ...
    try:
        ...
        while True:
            ...
            if records:
                tprint(f"🔍 Found {len(records)} LF records ready for frame creation")
